    ...
    <aside className="flex h-full min-h-0 w-full flex-col border-r border-border/50 bg-card/40 px-3 py-4 backdrop-blur-sm">
      ...
      <div className="mt-2 min-h-0 flex-1 space-y-1 overflow-y-auto pr-1 scrollbar">
        {conversations.length === 0 ? (
          <p className="px-2 py-4 text-[12px] leading-5 text-muted-foreground">Your research history appears here.</p>
        ) : conversations.map((conversation) => {
          const title = conversation.title || 'New research'
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className="mr-1 rounded-md p-1.5 text-muted-foreground/50 opacity-0 transition-all hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100 focus:opacity-100"
