      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <div className="text-[12px] text-muted-foreground mb-1">Stock</div>
            ...
            <div>
              <div className="text-[12px] text-muted-foreground mb-1">Rule name</div>
              <Input value={form.name} onChange={e => setForm(prev => ({ ...prev, name: e.target.value }))} placeholder="e.g. Breaks above 1500" />
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div>
              <div className="text-[12px] text-muted-foreground mb-1">Combine conditions</div>
            ...
            <div>
              <div className="text-[12px] text-muted-foreground mb-1">Active hours</div>
              <Select value={form.market_hours_mode} onValueChange={(v) => setForm(prev => ({ ...prev, market_hours_mode: v as any }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="trading_only">Market hours only</SelectItem>
                  <SelectItem value="always">All day</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div>
              <div className="text-[12px] text-muted-foreground mb-1">Cooldown (minutes)</div>
            ...
            <div>
              <div className="text-[12px] text-muted-foreground mb-1">Daily limit</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <div className="text-[12px] text-muted-foreground mb-1">Trigger mode</div>
              <Select value={form.repeat_mode} onValueChange={(v) => setForm(prev => ({ ...prev, repeat_mode: v as any }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="repeat">Can repeat</SelectItem>
                  <SelectItem value="once">Once only</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div>
              <div className="text-[12px] text-muted-foreground mb-1">Expires (optional)</div>
              ...
              <div className="mt-1 text-[10px] text-muted-foreground/70">Leave empty to never expire</div>
            </div>
          </div>

          <div className="rounded-lg border border-border/40 p-3">
            <div className="text-[12px] text-muted-foreground mb-2">Notification channels (none = system default)</div>
            {enabledChannels.length === 0 ? (
              <div className="text-[12px] text-muted-foreground/70">No channels available</div>
            ) : (
          ...
          <div className="rounded-lg border border-border/40 p-3 space-y-2">
            <div className="flex items-center justify-between">
              <div className="text-[12px] text-muted-foreground">Conditions</div>
              <Button variant="secondary" size="sm" className="h-7 text-[11px]" onClick={addCond}>Add condition</Button>
            </div>
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <Button onClick={submit} disabled={props.submitting}>
              {props.submitting ? 'Saving...' : (props.submitLabel || 'Save rule')}
            </Button>
          </div>
        </div>
      </DialogContent>
